    ...
    >
      ...
      <div
        ...
        style={{
          ...
          backgroundColor: hovered ? `${d.realmColor}40` : `${d.realmColor}20`,
          boxShadow: hovered ? `0 0 20px ${d.realmColor}40` : `0 0 8px ${d.realmColor}15`,
      ...
      {hovered && (
        <div
          className="absolute z-50 -top-16 left-1/2 -translate-x-1/2 rounded-lg border border-border/80 bg-card/95 backdrop-blur-sm px-3 py-2 shadow-xl whitespace-nowrap pointer-events-none"
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-border/60 bg-card/50 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-muted-foreground">({data.nodes.length} nodes • {data.edges.length} connections)</span>
        </div>
        <LanguageSwitcher />
      </div>
      ...
      <div className="flex-1" style={{ height: 'calc(100vh - 120px)' }}>
        ...
        >
          ...
          <Controls
            ...
            className="!bg-card/90 !border-border/60 !shadow-lg [&>button]:!bg-card/90 [&>button]:!border-border/60 [&>button]:!text-foreground [&>button:hover]:!bg-accent"
          />
          <MiniMap
            nodeColor={(node) => {
              const d = node.data as unknown as MapNodeData
              return d?.realmColor || '#10B981'
            }}
      ...
      {/* Realm Legend */}
      <div className="border-t border-border/60 bg-card/50 backdrop-blur-sm px-4 sm:px-6 py-3">
